...
static int __init cdrv_init(void) {
  ...
  // Получить у системы регион номеров для драйвера
  ...
  // Создать структуру под виртуальное устройство
  cdev_init(&my_dev, &cdrv_fops);  // хоть тут ничего не нужно проверять, уф
  my_dev.owner = THIS_MODULE;
  printk(KERN_INFO "Device structure created\n");


  printk(KERN_INFO "Adding virtual char device...\n");
...
static void __exit cdrv_exit(void) {
  cdev_del(&my_dev);
  printk(KERN_INFO "Virtual device deleted\n");
